...
async def get_unique_filename(dest_dir: Path, filename: str) -> str:
    """
    Generates a unique filename in the given directory if a file with that name already exists.
    ...
    Returns:
        str: A unique filename that doesn't conflict with existing files in the destination directory.
    """
...
async def copy_file(
    file_path: Path, dest_dir: Path, semaphore: asyncio.Semaphore, dry_run: bool = False
) -> None:
    """
    Copies the specified file to a subdirectory in the destination directory named after the file's extension.
    A semaphore limits the number of concurrent operations. If `dry_run` is True, the file is not actually copied.
    ...
    Args:
        ...
        dry_run (bool, optional): If True, the operation is simulated without copying the file. Defaults to False.
    ...
    async with semaphore:
        try:
            ...
            try:
                await asyncio.to_thread(new_dir.mkdir, exist_ok=True, parents=True)
            except PermissionError as e:
                logger.error(f"Permission error creating directory {new_dir}: {e}")
                return
            except Exception as e:
                logger.error(f"Unknown error creating directory {new_dir}: {e}")
                return
            ...
            if dry_run:
                logger.info(f"[DRY-RUN] Would copy {file_path} to {new_file_path}")
                return
            ...
            try:
                ...
                logger.error(
                    f"Access denied copying {file_path} to {new_file_path}: {e}"
                )
            except Exception as e:
                logger.error(
                    f"Unknown error copying {file_path} to {new_file_path}: {e}"
                )
...
async def read_folder(
    ...
    """
    Recursively reads the source directory and copies all found files to the destination directory,
    sorting them by their extension into subdirectories. Uses a semaphore to control concurrency.
    ...
    Args:
        ...
        semaphore (asyncio.Semaphore): Semaphore to limit concurrent operations.
        dry_run (bool, optional): If True, simulate operations without copying files. Defaults to False.
...
async def main() -> None:
    """
    The main asynchronous entry point of the program. It parses command-line arguments,
    sets up logging, checks directories, and then starts recursive file copying.
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Limit the number of concurrent file copy operations (default: 10).",
    )
    parser.add_argument(
        "--path-to-log",
        default="task1.log",
    ...
    try:
        ...
        logger.error(f"Permission error creating destination directory {dest_dir}: {e}")
        return
    except Exception as e:
        logger.error(f"Unknown error creating destination directory {dest_dir}: {e}")
        return

    logger.info(
        f"Starting copying from {source_dir} to {dest_dir}. Dry run: {args.dry_run}"
    )
